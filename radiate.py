...
class Sequence:
    ...
    def overlay_radar_lidar(self, radar, lidar):
        """
        Method that joins camera and projected lidar in one image for visualisation
    ...
    def vis_all(self, output, wait_time=1):
        ...
        if (output != {}):
            ...
            if (self.config['use_lidar_bev_image']):
                ...
                #cv2.imshow('lidar bev image', lidar_vis)
                if self.config['save_images']:
                    cv2.imwrite(os.path.join(self.output_folder,  str(
            ...
            #combination = 1*output['sensors']['lidar_bev_image'] + 1*output['sensors']['radar_cartesian'] + 0
            ...
            common_height = min(radar_cart_vis.shape[0], lidar_vis.shape[0],right_bb.shape[0])
            radar_resized = cv2.resize(radar_cart_vis, (int(radar_cart_vis.shape[1] * (common_height / radar_cart_vis.shape[0])), common_height))
            camera_resized = cv2.resize(right_bb, (int(right_bb.shape[1] * (common_height / right_bb.shape[0])), common_height))
            lidar_resized = cv2.resize(lidar_vis, (int(lidar_vis.shape[1] * (common_height / lidar_vis.shape[0])), common_height))
            ...
            cv2.imshow("Ground Truth Images", combined_image_horizontal)
